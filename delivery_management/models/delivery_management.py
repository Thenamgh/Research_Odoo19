...
class DeliveryManagement(models.Model):
    ...
    name = fields.Char(string='Delivery Reference', required=True, copy=False, readonly=True, default=lambda self: 'NEW')
    
    delivery_date = fields.Datetime(string='Delivery Date', required=True, default=fields.Datetime.now)
    ...
    delivery_lines = fields.One2many('delivery.line', 'delivery_id', string='Delivery Items')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'NEW') == 'NEW':
                vals['name'] = self.env['ir.sequence'].next_by_code('delivery.management') or 'DM/' + datetime.now().strftime('%Y%m%d%H%M%S')
