...
class DeliveryStatistics(models.Model):
    ...
    total_deliveries = fields.Integer(
        string="Total Deliveries", compute="_compute_statistics"
    )
    completed_deliveries = fields.Integer(
        string="Completed Deliveries", compute="_compute_statistics"
    )
    pending_deliveries = fields.Integer(
        string="Pending Deliveries", compute="_compute_statistics"
    )
    ...
    def _compute_statistics(self):
        for record in self:
            ...
            if record.period == "today":
                domain = [
                    ("created_date", ">=", today.replace(hour=0, minute=0, second=0))
                ]
            elif record.period == "week":
                start = today - timedelta(days=today.weekday())
                domain = [
                    ("created_date", ">=", start.replace(hour=0, minute=0, second=0))
                ]
            else:  # month
                domain = [
                    (
            ...
            record.completed_deliveries = len(
                deliveries.filtered(lambda d: d.state == "completed")
            )
            record.pending_deliveries = len(
                deliveries.filtered(lambda d: d.state in ["draft", "confirmed"])
            )
