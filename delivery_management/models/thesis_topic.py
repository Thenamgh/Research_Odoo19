...
class ThesisTopic(models.Model):
    ...
    assigned_count = fields.Integer(
        string="Số sinh viên đã nhận", compute="_compute_assigned_count", store=False
    )
    available_slots = fields.Integer(
        string="Vị trí còn trống", compute="_compute_available_slots", store=False
    )
    ...
    advisor_id = fields.Many2one(
        "res.users", string="Giảng viên hướng dẫn", required=True
    )
    ...
    assignment_ids = fields.One2many(
        "thesis.assignment", "topic_id", string="Danh sách sinh viên nhận"
    )

    def _compute_assigned_count(self):
        for topic in self:
            topic.assigned_count = len(
                topic.assignment_ids.filtered(lambda x: x.state == "assigned")
            )
    ...
    def get_available_topics(self):
        ...
        return self.search([("state", "=", "available"), ("available_slots", ">", 0)])
    ...
    def can_student_receive_topic(self, student_id):
        ...
        assignment = self.env["thesis.assignment"].search(
            [("student_id", "=", student_id), ("state", "=", "assigned")]
        )
...
class ThesisAssignment(models.Model):
    ...
    topic_id = fields.Many2one(
        "thesis.topic", string="Đề tài", required=True, ondelete="cascade"
    )
    ...
    assigned_date = fields.Date(
        string="Ngày giao", default=fields.Date.today, readonly=True
    )
